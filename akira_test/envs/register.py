...
class EnvSpec(object):
    ...
    def __init__(self, id, entry_point=None, reward_threshold=None, nondeterministic=False, max_episode_steps=None, kwargs=None):
        ...
        if not match:
            raise ValueError('Attempted to register malformed environment ID: {}. (Currently all IDs must be of the form {}.)'.format(
...
class EnvRegistry(object):
    ...
    def spec(self, path):
        ...
        match = env_id_re.search(id)
        if not match:
            raise ValueError('Attempted to look up malformed environment ID: {}. (Currently all IDs must be of the form {}.)'.format(
    ...
    def register(self, id, **kwargs):
        if id in self.env_specs:
            raise ValueError('Cannot re-register id: {}'.format(id))
