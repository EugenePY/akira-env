...
def is_schema_decorator(attr_name, attr):
    return hasattr(attr, "__marshmallow_hook__")
...
class ModelMeta(type):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        model_class = super(ModelMeta, mcs).__new__(
            mcs, name, bases, model_attrs)
        ...
        _schema_class = SchemaMeta('%sSchema' %
                                   name, (schema_base,), schema_attrs)
...
class Model(with_metaclass(ModelMeta, ModelABC)):
    ...
    def __init__(self, _raw_data=None, **kwargs):
        ...
        if not isinstance(_raw_data, (dict, type(None))):
            raise ValueError('%s constructor accepts dictionary or kwargs. Got:%s' % (
                self.__class__.__name__, type(_raw_data)))
        constructor_schema = self._schema_class()
        ...
        self.__dict__.update(constructor_schema.load(input_))
    ...
    def __setattr__(self, key, value):
        ...
        if isinstance(attr, FieldABC) and not isinstance(attr, Nested):
            ...
            deserialized_value = attr.deserialize(value, key, {key: value})
            return super(Model, self).__setattr__(key, deserialized_value)
        else:
            return super(Model, self).__setattr__(key, value)
    ...
    def save(self, db):
        tb = db.table(self.__tablename__,
                      model_cls=self.__class__)
        return tb.insert(self.dump())


# noinspection PyPep8Naming
def NestedModel(model_cls):
    ...
    for att_name, att_method in model_cls.__dict__.items():
        # make the nesting Model as Nesting field
        pass
    return model_cls
